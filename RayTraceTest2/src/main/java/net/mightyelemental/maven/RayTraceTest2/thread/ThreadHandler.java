...
public class ThreadHandler {
	...
	public ThreadHandler(int chunkWidth, int chunkHeight, int screenWidth, int screenHeight) {
	...
	private void defineChunks(int chunkWidth, int chunkHeight, int screenWidth, int screenHeight) {
		for (int x = 0; x < screenWidth; x += chunkWidth) {
			for (int y = 0; y < screenHeight; y += chunkHeight) {
				int height = Math.min(screenHeight - (y + chunkHeight), chunkHeight);
				int width = Math.min(screenWidth - (x + chunkWidth), chunkWidth);
	...
	public List<Thread> setupRenderingThreads(App a, int[] pixels, int pixelSize) {
		...
		for (int core = 0; core < Properties.cores; core++) {
				...
				public void run() {
					for (RenderChunk c : chunks) {
						if (!c.startedRender) {
							...
							for (int x = c.x; x < c.x + c.width; x += pixelSize) {
								for (int y = c.y; y < c.y + c.height; y += pixelSize) {
