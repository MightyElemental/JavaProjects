	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		switch (menuState) {
			...
			case STATE_UPGRADE:
				...
				g.drawImage(purchase.boughtMowers.get(upgradeButtons.mowerNumber).getDisplayIcon().getScaledCopy(h - 1, h - 1), x, y + 1);
				break;
		}
		renderMoney(gc, sbg, g);
		mowerInfo.draw(gc, sbg, g, purchase.boughtMowers.get(upgradeButtons.mowerNumber), 50);
	}
	...
	public void onButtonPushed(Button b, int button) {
		...
		if (b.equals(upgradeButtons.durability)) {
			if (purchase.durabilityLevel < 4) {
				...
				upgradeButtons.durability.setText("Upgrade Durability (" + MowerGame.gameState.worldObj.lawnMower.maxHealth + "+10)");
			}
		}
