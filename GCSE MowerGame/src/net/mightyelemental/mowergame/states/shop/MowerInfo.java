...
public class MowerInfo {
	...
	private String[] splitEverNumChars(String text, int n) {
		char[] chars = new char[n];
		Arrays.fill(chars, '.');
		String result = new String(chars);
		return text.split("(?<=\\G" + result + ")");
	}

	private void drawDescription(Graphics g, MowerType mower, float y) {//Fix word splitting
		String desc = mower.getDescription();
		String[] lines = splitEverNumChars(desc, 21);
		g.setColor(Color.black);
