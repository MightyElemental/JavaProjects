...
public class LoadState extends BasicGameState {
	...
	public LoadState(int statePreLoad) {
		this.ID = statePreLoad;
	}

	@Override
	public void init(GameContainer gc, StateBasedGame sbg) throws SlickException {
		companyLogo = MowerGame.resLoader.loadImage("IridiumGames");
	}
	...
	public void render(GameContainer gc, StateBasedGame sbg, Graphics g) throws SlickException {
		...
		if (alpha < 1f) {
			g.drawImage(temp, (gc.getWidth() / 2) - (width / 2), (gc.getHeight() / 2) - (height / 2),
	...
	private float xShift = (rand.nextInt(60) - 30), yShift = (rand.nextInt(60) - 30),
			alphaShift = rand.nextInt(10) / 10f;

	public void renderImageWithGlitches(GameContainer gc, StateBasedGame sbg, Graphics g, Image temp, float width,
			float height) {
