...
public class ScrollBar extends GUIObject {
	...
	public void draw(Graphics g) {
		...
		g.draw(this);
		g.drawString(getText(), this.getCenterX() - g.getFont().getWidth(getText()) / 2,
				getCenterY() - g.getFont().getHeight(getText()) / 2);
		int temp = Math.round(getValue());
		g.drawString(temp + "", getWidth() + getX() - g.getFont().getWidth(temp + "") - 20,
				getCenterY() - g.getFont().getHeight(temp + "") / 2);
	}
