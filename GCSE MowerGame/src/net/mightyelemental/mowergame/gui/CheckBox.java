...
public class CheckBox extends GUIObject {
	...
	public CheckBox(float x, float y, Graphics g) {
		super(x, y, 40, 40);
		box = new Rectangle(x, y, 40, 40);
		textY = (int) (box.getCenterY() - g.getFont().getHeight(getText()) / 2);
		textX = (int) (box.getMaxX() + 10);
		this.setWidth(box.getWidth() + 10 + g.getFont().getWidth(getText()));
	}
	 ...
	 *            the text to set
	 */
	public CheckBox setText(String text, Graphics g) {
		super.setText(text);
		this.setWidth(g.getFont().getWidth(getText()) + 10 + box.getWidth());
		return this;
	}
